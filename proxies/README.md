...
Each subdirectory is a self-contained proxy implementation. At stack creation, the
directory matching the `ProxyEngine` parameter is seeded into the CodeCommit
repository, which becomes the editable source of truth; commits there trigger the
pipeline to build and deploy a new image.
...
Each directory contains the `Dockerfile` (engine version pinned via a build arg), the
proxy config (`nginx.conf` / `envoy.yaml`), and a `buildspec.yml` that builds the image
with an immutable commit-based tag and emits `imagedefinitions.json` for the
CodePipeline ECS deploy action.

For the engine comparison and how to choose, see the repository
[README](../README.md#proxy-engines). For optional HTTP listeners, see
...
per-task CloudWatch Logs group.

## NGINX

Based on the official `nginx` Alpine image (from Amazon ECR Public, which avoids Docker
Hub pull-rate limits in the build), pinned via the `NGINX_VERSION` build arg in the
[Dockerfile](./nginx/Dockerfile). The Alpine image has the stream module compiled in, so
there is **no `load_module` directive** in [`nginx.conf`](./nginx/nginx.conf). The
`stream` listener does the passthrough:
...
`listen 443 proxy_protocol` trusts the NLB to pass the true source IP
(`set_real_ip_from 192.168.0.0/16`), and a `map` on `$bytes_received` keeps health-check
noise out of the access log.
...
This is the Envoy equivalent of NGINX's `proxy_pass $ssl_preread_server_name`. Access
logs go to stdout (and on to CloudWatch); the admin interface is bound to localhost for
debugging via ECS Exec.

> The SNI dynamic forward proxy filter is currently labelled *alpha* by Envoy. The base
> image is pulled from Docker Hub (no ECR Public mirror), so a heavily-throttled build
> may need Docker Hub authentication or an ECR mirror.
